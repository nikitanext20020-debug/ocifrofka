  ...
  const normalizedRows = rows.map((source, index) => {
    ...
    for (const { column, values } of categoricalColumns) {
      ...
      if (canonical) {
        ...
        // Fallback: use first allowed value instead of crashing with 502
        row[column] = values[0] || "";
        normalizationIssues.push(`значение «${row[column]}» в колонке «${body.headers[column]}» заменено на допустимое из списка`);
      }
    }
      ...
      messages: [
        {
          ...
          content: [
            ...
            " Каждый текст должен быть экстремально коротким: ровно два предложения, суммарно не более 10-12 слов на всё обращение. Например: «Яма на дороге. Прошу заасфальтировать.» или «Нет света в подъезде. Разберитесь пожалуйста.» Не пиши лишних слов и подробностей.",
            ...
            " Тема текста относится только к содержанию колонки \"Текст наказа\"; для колонок \"Тематика предложения\" и \"Направление обращения\" выбирай СТРОГО И ТОЛЬКО из предоставленного списка allowedValues. Не придумывай свои категории.",
